...
class AccountGroupDialogPresenter:
    ...
    def run_add_dialog(self) -> None:
        ...
        logging.debug("Running AccountGroupDialog (edit=False)")
        self._dialog.exec()
    ...
    def run_edit_dialog(self) -> None:
        ...
        logging.debug("Running AccountGroupDialog (edit=True)")
        self._dialog.exec()
    ...
    def add_account_group(self) -> None:
        ...
        logging.info(f"Adding AccountGroup: {path=}, {index=}")
        try:
        ...
        new_account_group = self._record_keeper.get_account_group(path)
        index_actual = self._get_item_index(new_account_group)
    ...
    def edit_account_group(self) -> None:
        ...
        logging.info(
            f"Editing AccountGroup at path='{item.path}', index={previous_index}: "
        ...
        edit_account_group = self._record_keeper.get_account_group(new_path)
        new_index_actual = self._get_item_index(edit_account_group)
    ...
    def _update_edit_dialog_position_limits(self, path: str) -> None:
        ...
        try:
            current_account_group = self._record_keeper.get_account_group(current_path)
        except NotFoundError:
            return
        parent_path, _, _ = path.rpartition("/")
        ...
        if not parent_path:
            ...
            if current_account_group not in self._record_keeper.root_account_items:
                maximum_position += 1
        else:
            try:
                parent = self._record_keeper.get_account_group(parent_path)
                maximum_position = len(parent.children)
                if current_account_group not in parent.children:
                    maximum_position += 1
            except NotFoundError:
                maximum_position = 1
