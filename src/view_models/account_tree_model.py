    ...
    for item in items:
        ...
        if node is None:
            try:
                ...
                balance_base = None
            if (
                isinstance(item, CashAccount)
                or (isinstance(item, SecurityAccount) and item.currency is not None)
                or (isinstance(item, AccountGroup) and item.currency is not None)
            ):
