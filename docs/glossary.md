...
There are two types of Accounts: [Cash Accounts](#cash-account-icon), which contain money denominated in a certain [Currency](#currency-icon), and [Security Accounts](#security-account-icon), which contain shares of [Securities](#security-icon).

In Kapytal, Accounts are disambiguated by their [path](#path). Account hierarchy can be created within [Account Tree](#account-tree-icon) using [Account Groups](#account-group-icon).

## Account Group ![Icon](../resources/icons/icons-16/folder.png)

Account Groups are folders containing [Accounts](#account) or other Account Groups. There is no limit to the depth of nesting within the [Account Tree](#account-tree-icon). Account Groups are disambiguated by their [path](#path).

## Account Item

Account Item is an umbrella term for [Accounts](#account) and [Account Groups](#account-group-icon).

## Account Tree ![Icon](../resources/icons/icons-16/folder-tree.png)

Account Tree is the second largest user interface feature of Kapytal, and it is the used to create, browse, edit, delete or otherwise manipulate [Accounts](#account) and [Account Groups](#account-group-icon). It is also the place where the balances of all Accounts and Account Groups are visible in both their [native](#native-currencyamount) and [base](#base-currency) [Currencies](#currency-icon).

Account Tree also offers an intuitive and easy to use interface shortcut to the Account [Transaction Filter](#transaction-filters-icon) via the check boxes in the rightmost Account Tree column. Only [Transactions](#transaction) related to the currently checked [Account Items](#account-item) are shown in the [Transaction Table](#transaction-table-icon).
...
In Kapytal, exactly one Currency at a time can be designated as base Currency. Base Currency is used in Base Balance column and Checked Account Balance widget in [Account Tree](#account-tree-icon), Base Amount column and Selected Transactions Total widget in [Transaction Table](#transaction-table-icon), and in [Reports](#reports).
...
Buy is a sub-type of [Security Transaction](#security-transaction) that represents the exchange of money from a [Cash Account](#cash-account-icon) for a gain of shares of a [Security](#security-icon) in a [Security Account](#security-account-icon).

## Cash

In Kapytal, Cash is synonym for money denominated in a certain [Currency](#currency-icon), regardless of whether it is physical (coins, bank notes) or digital (bank accounts, cryptocurrencies etc.). Kapytal is not capable of making a distinction between a physical or digital money.

Related terms are: [Cash Account](#cash-account-icon), [Cash Transaction](#cash-transaction) and [Cash Transfer](#cash-transfer-icon).

## Cash Account ![Icon](../resources/icons/icons-16/piggy-bank.png)

Cash Account is a type of [Account](#account) that can contain any amount of money denominated in a certain [Currency](#currency-icon). When creating a Cash Account, its native [Currency](#currency-icon) is defined by the user and can never be changed.
...
Cash Accounts are used in all types of [Transactions](#transaction) except [Security Transfers](#security-transfer-icon).
...
In Table tab, double-clicking on any table cell (or selecting a table cell and selecting ![Icon](../resources/icons/icons-16/table.png) "Show Transactions" action) brings up a small version of a [Transaction Table](#transaction-table-icon) with all the [Transactions](#transaction) that were used for calculating the value of that table cell. The Transactions can be edited from this small Transaction Table.

Evaluated quantities are:

- Income
- Inward Transfers
  - [Cash Transfers](#cash-transfer-icon), [Security Transactions](#security-transaction) and [Security Transfers](#security-transfer-icon) transferring value from an unselected [Cash Account](#cash-account-icon) or [Security Account](#security-account-icon) to one of the selected [Account Items](#account-item)
- Refunds
- Initial Balances
  - see [Cash Account](#cash-account-icon) for more info
- Total Inflow
- Expenses
- Outward Transfers
...
Cash Transactions are [Transactions](#transaction) that represent the transfer of money between a [Cash Account](#cash-account-icon) and a [Payee](#payee-icon). There are two types of Cash Transactions: [Income](#income-icon) and [Expense](#expense-icon).

Cash Transactions can contain multiple Categories (so-called "split Categories").

Cash Transaction specific attributes are: [Cash Account](#cash-account-icon), [Payee](#payee-icon), Amount and [Categories](#category-icon).

## Cash Transfer ![Icon](../resources/icons/icons-custom/coins-arrow.png)

Cash Transfers are [Transactions](#transaction) that can transfer money from one [Cash Account](#cash-account-icon) to another. The [Currencies](#currency-icon) of the Cash Accounts do not have to match, and Cash Transfers can therefore represent Currency exchange operations.
...
Categories are attributes that can be used in [Cash Transactions](#cash-transaction) and [Refunds](#refund-icon). The user can create any amount of Categories, and Categories can form a tree-like hierarchy. The user can use Categories to filter [Transactions](#transaction) or create specific Category [Reports](#reports).

Each Category is defined by its [path](#path) and type (Income, Expense, Income and Expense). Income Categories can only be used in [Income](#income-icon) Cash Transactions, Expense Categories can only be used in [Expense](#expense-icon) Cash Transactions and Income and Expense Categories can be used in any Cash Transactions.

Categories are meant to be rather generic and "reused" often. [Tags](#tag-icon) are better suited as an "one-off" attribute.
...
Currencies and Exchange Rates Form is the [Form](#form) for creating, editing, deleting, manipulating and updating [Currencies](#currency-icon), [Exchange Rates](#exchange-rate-icon) and Exchange Rate quotes.
...
Exchange Rates relate two [Currencies](#currency-icon) together through a numerical conversion factor. Exchange Rates can be created within [Currencies and Exchange Rates Form](#currencies-and-exchange-rates-form).
...
Expense is a sub-type of [Cash Transaction](#cash-transaction), which represents the transfer of money from a [Cash Account](#cash-account-icon) to a [Payee](#payee-icon). Unlike [Income](#income-icon), Expense can be refunded by a [Refund](#refund-icon). Once an Expense is refunded, it cannot be edited anymore.
...
- [Category](#category-icon) Form
- [Currencies and Exchange Rates Form](#currencies-and-exchange-rates-form)
- [Payee](#payee-icon) Form
- [Update Quotes Form](#update-quotes-form-icon)
- [Securities Form](#securities-form)
- [Settings Form](#settings-form)
- [Tags](#tag-icon) Form
- [Transaction Filter](#transaction-filters-icon) Form

## Income ![Icon](../resources/icons/icons-custom/coins-plus.png)

Income is a sub-type of [Cash Transaction](#cash-transaction), which represents the transfer of money from a [Payee](#payee-icon) to a [Cash Account](#cash-account-icon). Unlike [Expense](#expense-icon), Income cannot be [refunded](#refund-icon).
...
The [Currency](#currency-icon) of a [Cash Account](#cash-account-icon) is considered to be its native Currency.

In the [Account Tree](#account-tree-icon), the balance of all Cash Accounts is shown in both native and [base](#base-currency) Currency, unless the native Currency of the particular Cash Account matches the base Currency. In that case, native balance is empty (as it would be identical to base amount anyway).

In the [Transaction Table](#transaction-table-icon), Native amount and Base amount are two separate columns. Native amount displays the amount of all [Transactions](#transaction) except the [Cash Transfer](#cash-transfer-icon) and [Security Transfer](#security-transfer-icon) Transactions, which are denominated in a Currency different from the base Currency.
...
- Accounts
  - used for visualizing how much each Account Item from [Account Tree](#account-tree-icon) is worth
- Asset Type
  - used for calculating and visualizing how much net worth within selected Account Items consists of money denominated in specific [Currencies](#currency-icon) or money tied up in specific [Securities](#security-icon)
- Time
  - used for visualizing the time evolution of selected Account Items

## Path

In Kapytal, items in tree-like hierarchies ([Account Items](#account-item) or [Categories](#category-icon)) are identified by their path. The path string consists of item names separated by slashes, starting from root item: `Grandparent Name/Parent Name/Item Name`.
...
In case of [Income](#income-icon), the Payee could be an employer, customer, the bank paying the user savings account interest, the government sending them the tax refund etc. In case of [Expense](#expense-icon), the Payee could be a landlord, telephone company, local grocery shop, gym etc.
...
Refund is a special type of [Transaction](#transaction) that "reverts" the effects of an [Expense](#expense-icon). The user can create a Refund by right-clicking an Expense in [Transaction Table](#transaction-table-icon) and selecting "Add Refund".
...
Refunds can refund money to a different [Cash Account](#cash-account-icon) than the Cash Account defined in the refunded Expense. The date & time of a Refund cannot be earlier than the date & time of the refunded Expense.
...
For the purposes of most [Reports](#reports), Refunds are either treated separately, or as "inverted" Expense. Refunds are never considered as [Income](#income-icon).
...
Note reports always analyse only the [Transactions](#transaction) or [Account Items](#account-item) that are passing through [Transaction Filters](#transaction-filters-icon). The user can use this to their advantage, for example to remove some Transactions from the analysed data set etc.

There are several types of Reports:

- [Net Worth Reports](#net-worth-reports-icon)
- [Cash Flow Reports](#cash-flow-reports-icon)
- Category Reports
- Tags Reports
- Payee Reports
...
Savings Rate is a quantity evaluated in [Cash Flow Reports](#cash-flow-reports-icon).

Savings Rate quantifies saved money relative to the sum of all saveable money for a given period. [Refunds](#refund-icon) contribute towards the saved amount, but not towards the saveable amount, therefore Savings Rate of more than 100% is theoretically possible.
...
In Kapytal, shares of Securities can be owned within [Security Accounts](#security-account-icon), can be acquired or sold via [Security Transactions](#security-transaction), and can be transfered to other Security Accounts via [Security Transfers](#security-transfer-icon).

Price history of Securities can be tracked by Kapytal. Prices for a specific calendar day can be entered manually or loaded from a CSV file in [Securities Form](#securities-form), or updated automatically in [Update Quotes Form](#update-quotes-form-icon).

When creating a Security in Securities Form, following attributes are requested:

- Name
- Symbol
  - optional, but required for automatic price updates via [Update Quotes Form](#update-quotes-form-icon)
- Type
...
Security Account is a type of [Account](#account) that can contain shares of [Securities](#security-icon). Unlike [Cash Accounts](#cash-account-icon), which can contain only money of single [Currency](#currency-icon), Security Accounts can contain shares of any number of various Securities, and the Securities do not even have to be denominated in the same Currency.

There is no Currency associated with Security Accounts. In [Account Tree](#account-tree-icon), their balance is shown only in terms of the [base Currency](#base-currency).
...
Security Accounts are used in [Security Transactions](#security-transaction) and [Security Transfers](#security-transfer-icon).

## Security Transaction

Security Transactions are [Transactions](#transaction) that represent buying or selling shares of a [Security](#security-icon) in exchange for monetary value. There are two sub-types of Security Transactions: [Buy](#buy-icon) and [Sell](#sell-icon).

Security Transaction specific attributes are: [Cash Account](#cash-account-icon), [Security Account](#security-account-icon), Security, shares, price per share.

## Security Transfer ![Icon](../resources/icons/icons-custom/certificate-arrow.png)

Security Transfers are [Transaction](#transaction) that can transfer shares of a [Security](#security-icon) from one [Security Account](#security-account-icon) to another.
...
Sell is a sub-type of [Security Transaction](#security-transaction) that represents the exchange of shares of a [Security](#security-icon) from a [Security Account](#security-account-icon) for a gain of money in a [Cash Account](#cash-account-icon).
...
Tags are not meant for describing the object of the Transactions (i.e. "Groceries", "Electronics", "Rent" etc.) as that is the purpose of [Categories](#category-icon).

## Transaction

Transactions are used to represent all real-world Transactions the user has made.

There are several types of Transactions: [Cash Transactions](#cash-transaction), [Cash Transfers](#cash-transfer-icon), [Refunds](#refund-icon), [Security Transactions](#security-transaction) and [Security Transfers](#security-transfer-icon).

All Transaction types contain the following attributes: [Date](#date), Description and [Tags](#tag-icon).

## Transaction Filters ![Icon](../resources/icons/icons-16/funnel.png)

Transaction Filters are used to filter [Transactions](#transaction) based on specific criteria. Transaction Filter Form can be accessed by clicking on the funnel icon next to the [Transaction Table](#transaction-table-icon) search bar.
...
- Description Filter
- Account Filter
  - mostly accessed directly from the [Account Tree](#account-tree-icon) check boxes
  - it filters Transactions based on the [Account Items](#account-item) they relate to, but also the Account Items themselves (mainly for [Net Worth Report](#net-worth-reports-icon) purposes)
- Tag Filters
