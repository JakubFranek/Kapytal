...
from src.models.transactions.attributes.attribute import TransactionAttribute
...
def test_name_not_string(name: Any) -> None:
    with pytest.raises(TypeError, match="TransactionAttribute name must be a string."):
        TransactionAttribute(name)


@given(name=st.just(""))
def test_name_too_short(name: str) -> None:
    with pytest.raises(ValueError, match="TransactionAttribute name length must be*"):
        TransactionAttribute(name)
