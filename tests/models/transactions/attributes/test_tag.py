...
from src.models.transactions.attributes.tag import Tag
...
@st.composite
def tags(draw: Callable[[st.SearchStrategy[str]], str]) -> Tag:
...
def test_creation_pass(name: str) -> None:
    ...
    dt_created_diff = tag.date_created - dt_start
    dt_edited_diff = tag.date_last_edited - dt_start

    assert tag.name == name
    assert dt_created_diff.seconds < 1
    assert dt_created_diff == dt_edited_diff
    assert tag.total_expense == 0
    assert tag.total_income == 0
    assert tag.total_sum == 0
    assert tag.total_volume == 0
...
def test_name_too_short(name: str) -> None:
    with pytest.raises(ValueError, match="Tag name length must be*"):
        Tag(name)
...
def test_date_last_edited(name: str, new_name: str) -> None:
    ...
    assert tag.date_created != tag.date_last_edited
...
def test_update_totals(
    ...
    tag = Tag(name)
    tag.update_totals(amount_1, transaction_type_1)
    tag.update_totals(amount_2, transaction_type_2)
    ...
    assert tag.total_volume.quantize(quanta) == expected_volume.quantize(quanta)
    assert tag.total_sum.quantize(quanta) == expected_sum.quantize(quanta)
    assert tag.total_expense.quantize(quanta) == expected_expense.quantize(quanta)
    assert tag.total_income.quantize(quanta) == expected_income.quantize(quanta)
...
def test_update_totals_invalid_amount_type(
    tag: Tag, amount: Any, transaction_type: CashTransactionType
) -> None:
    with pytest.raises(TypeError, match="Update amount must be of type Decimal."):
        tag.update_totals(amount, transaction_type)
